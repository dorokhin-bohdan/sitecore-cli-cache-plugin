...
public class HtmlCacheCleanerTests : BaseCacheCleanerTests<HtmlCacheCleaner>
{
    protected override CacheType ExpectedCacheType => CacheType.Html;
    protected override Mock<HtmlCacheCleaner> CacheCleanerMock { get; }

    public HtmlCacheCleanerTests()
    {
        var bytesConverter = new Mock<IBytesConverter>();
        CacheCleanerMock = new Mock<HtmlCacheCleaner>(bytesConverter.Object)
        {
            CallBase = true
        };
    }
    ...
    public void GetCacheInfo_ShouldCallProperCache()
    {
        ...
        result.Should().Be(cachesMock.Object.HtmlCache.InnerCache);
    }
}